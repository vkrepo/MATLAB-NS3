...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<DoubleValue>*>(obj->vptr)));
    else
        return obj->vptr;
}
    ...
    switch(identifyMethod(cmd))
    {
        case DOUBLEVALUE:
        {
            ...
            switch(nrhs)
            {
                ...
                case 2:
                {
                    double const & arg2 = (double) *mxGetPr(prhs[1]);
                    DoubleValue *obj = new DoubleValue(arg2);
                    MemMngmt *tst = bindMngrNewElem();
                    tst->vptr = reinterpret_cast<void*>(obj);
                    tst->objType = DOUBLEVALOBJ;
                    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == DOUBLEVALOBJ)
                    delete reinterpret_cast<DoubleValue*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<DoubleValue>*>(obj->vptr);
