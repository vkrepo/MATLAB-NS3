...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<SsidValue>*>(obj->vptr)));
    else
        return obj->vptr;
}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    switch(identifyMethod(cmd))
    {
        case SSIDVALUE:
        {
            ...
            switch(nrhs)
            {
                ...
                case 2:
                {
                    ...
                    SsidValue *obj = new SsidValue(arg2);
                    MemMngmt *tst = bindMngrNewElem();
                    tst->vptr = reinterpret_cast<void*>(obj);
                    tst->objType = SSIDVALOBJ;
                    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == SSIDVALOBJ)
                    delete reinterpret_cast<SsidValue*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<SsidValue>*>(obj->vptr);
