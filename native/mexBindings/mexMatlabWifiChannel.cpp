...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<MatlabWifiChannel>*>(obj->vptr)));
    else
        return obj->vptr;
}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    switch(identifyMethod(cmd))
    {
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == MATLABCHANOBJ)
                    delete reinterpret_cast<MatlabWifiChannel*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<MatlabWifiChannel>*>(obj->vptr);
