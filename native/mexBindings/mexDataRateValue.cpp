...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<DataRateValue>*>(obj->vptr)));
    else
        return obj->vptr;
}
    ...
    switch(identifyMethod(cmd))
    {
        case DATARATEVALUE:
        {
            ...
            switch(nrhs)
            {
                case 1:
                {
                    DataRateValue *obj = new DataRateValue();
                ...
                case 2:
                {
                    ...
                    DataRate const & arg2 = *(reinterpret_cast<DataRate*>(tst2->vptr));
#else
                    double arg2 = *mxGetPr(prhs[1]);
#endif
                    DataRateValue *obj = new DataRateValue(arg2);
                    MemMngmt *tst = bindMngrNewElem();
                    tst->vptr = reinterpret_cast<void*>(obj);
                    tst->objType = DRVALUE;
                    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == DRVALUE)
                    delete reinterpret_cast<DataRateValue*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<DataRateValue>*>(obj->vptr);
