...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<ConstantAccelerationMobilityModel>*>(obj->vptr)));
    else
        return obj->vptr;
}
...
static char methodNames[TOTAL_METHODS][CMD_BUFFER_SIZE] = {"SetVelocityAndAcceleration", "new", "delete"};
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    {
        case SETVELOCITYANDACCELERATION:
        {
            if ((nlhs != 0) || (nrhs > 4))
            {
            ...
            Vector const & arg3 = convmxArrayToVector(prhs[3]);
            void *obj =  getElem(bindMngrHasListElem ((MemMngmt *)(*((uint64_t *)mxGetData(prhs[1])))));
            (reinterpret_cast<ConstantAccelerationMobilityModel*>(obj))->SetVelocityAndAcceleration(arg2, arg3);
            break;
        }
        case CONSTANTACCELERATIONMOBILITYMODEL:
        {
            if ((nlhs != 1) || (nrhs > 1))
            {
                mexErrMsgTxt("enter correct number of input and output args");
            }
            ConstantAccelerationMobilityModel *obj = new ConstantAccelerationMobilityModel();
            MemMngmt *tst = bindMngrNewElem();
            tst->vptr = reinterpret_cast<void*>(obj);
            tst->objType = CONSTAMMOBJ;
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == CONSTAMMOBJ)
                    delete reinterpret_cast<ConstantAccelerationMobilityModel*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<ConstantAccelerationMobilityModel>*>(obj->vptr);
