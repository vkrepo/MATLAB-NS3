...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<ConstantPositionMobilityModel>*>(obj->vptr)));
    else
        return obj->vptr;
}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    {
        case CONSTANTPOSITIONMOBILITYMODEL:
        {
            if ((nlhs != 1) || (nrhs > 1))
            {
                mexErrMsgTxt("enter correct number of input and output args");
            }
            ConstantPositionMobilityModel *obj = new ConstantPositionMobilityModel();
            MemMngmt *tst = bindMngrNewElem();
            tst->vptr = reinterpret_cast<void*>(obj);
            tst->objType = CONSTPMMOBJ;
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == CONSTPMMOBJ)
                    delete reinterpret_cast<ConstantPositionMobilityModel*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<ConstantPositionMobilityModel>*>(obj->vptr);
