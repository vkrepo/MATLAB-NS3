...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    switch(identifyMethod(cmd))
    {
        case SETATTRIBUTE:
        {
            ...
            (reinterpret_cast<PacketSinkHelper*>(obj))->SetAttribute(arg2, arg3);
            break;
        }
        case PACKETSINKHELPER:
        {
            if ((nlhs != 1) || (nrhs > 3))
            {
                mexErrMsgTxt("enter correct number of input and output args");
            ...
            PacketSinkHelper *obj = new PacketSinkHelper(arg2, arg3);
            MemMngmt *tst = bindMngrNewElem();
            tst->vptr = reinterpret_cast<void*>(obj);
            tst->objType = PKTSINKHELPER;
        ...
        case INSTALL:
        {
            ...
            switch(nrhs)
            {
                case 3:
                {
                    ...
                    ApplicationContainer *tmp = new ApplicationContainer((reinterpret_cast<PacketSinkHelper*>(obj))->Install(arg2));
#else
                    ApplicationContainer *tmp;
                    if (tst2->objType == NODECONTOBJ)
                    {
                        NodeContainer arg2 = *(reinterpret_cast<NodeContainer*>(tst2->vptr));
                        tmp = new ApplicationContainer((reinterpret_cast<PacketSinkHelper*>(obj))->Install(arg2));
                    }
                    else if (tst2->objType == NODEOBJ_SP)
                    {
                        ...
                        mexErrMsgTxt("error in packet sink helper installation");
#endif
                    MemMngmt *tst = bindMngrNewElem();
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == PKTSINKHELPER)
                    delete reinterpret_cast<PacketSinkHelper*>(obj->vptr);
