...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<StringValue>*>(obj->vptr)));
    else
        return obj->vptr;
}
    ...
    switch(identifyMethod(cmd))
    {
        case STRINGVALUE:
        {
            ...
            switch(nrhs)
            {
                ...
                case 2:
                {
                    std::string const & arg2 = std::string(mxArrayToString(prhs[1]));
                    StringValue *obj = new StringValue(arg2);
                    MemMngmt *tst = bindMngrNewElem();
                    tst->vptr = reinterpret_cast<void*>(obj);
                    tst->objType = STRINGVALOBJ;
                    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == STRINGVALOBJ)
                    delete reinterpret_cast<StringValue*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<StringValue>*>(obj->vptr);
