...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<UintegerValue>*>(obj->vptr)));
    else
        return obj->vptr;
}
    ...
    switch(identifyMethod(cmd))
    {
        case UINTEGERVALUE:
        {
            ...
            switch(nrhs)
            {
                ...
                case 2:
                {
                    uint64_t const & arg2 = (uint64_t) *mxGetPr(prhs[1]);
                    UintegerValue *obj = new UintegerValue(arg2);
                    MemMngmt *tst = bindMngrNewElem();
                    tst->vptr = reinterpret_cast<void*>(obj);
                    tst->objType = UINTEGERVALOBJ;
                    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == UINTEGERVALOBJ)
                    delete reinterpret_cast<UintegerValue*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<UintegerValue>*>(obj->vptr);
