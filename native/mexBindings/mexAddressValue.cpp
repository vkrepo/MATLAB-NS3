...
void* getElem(MemMngmt *obj)
{
    if (obj->objType >= SMART_PTR_ENUM_START )
        return (void*)PeekPointer(*(reinterpret_cast<Ptr<AddressValue>*>(obj->vptr)));
    else
        return obj->vptr;
}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    switch(identifyMethod(cmd))
    {
        case ADDRESSVALUE:
        {
            ...
            switch(nrhs)
            {
                ...
                case 2:
                {
                    ...
                    AddressValue *obj = new AddressValue(arg2);
                    MemMngmt *tst = bindMngrNewElem();
                    tst->vptr = reinterpret_cast<void*>(obj);
                    tst->objType = ADDRVALOBJ;
                    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
        ...
        case DELETE:
        {
            ...
            if (obj)
            {
                if (obj->objType == ADDRVALOBJ)
                    delete reinterpret_cast<AddressValue*>(obj->vptr);
                else
                    delete reinterpret_cast<Ptr<AddressValue>*>(obj->vptr);
